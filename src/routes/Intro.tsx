...
export const SignUpModal: FC<Props> = ({
  ...
  const { signUp, currentUser, signIn, signInViaGoogle, loading } =
    ...
    if (isRegisterForm) {
      ...
      return (
        <>
          ...
          <div
            className={`${styles.facebook} ${styles.google}`}
            onClick={handleLoginWithGoogle}
          >
            <img className={styles.icon} src={""} alt="google icon" />
      ...
      <div className={styles.box} onClick={onClose}>
        <div className={styles.container} onClick={(e) => e.stopPropagation()}>
          <h3 className={styles.title}>
            {isLoginForm ? "Login in the account" : "Sign up for an account"}
          </h3>
          {registerFormOrOAuth}
          {!isLoginForm && (
          ...
          <div className={styles.footer}>
            <span className={styles.accquestion}>
              {isLoginForm ? "Have not an account?" : "Have an account?"}
            </span>
